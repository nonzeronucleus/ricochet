...
var _square_size
var _square_views:MultiArray
var _all_robots:Array = []
var _level:Level
...
func reset():
	if _level:
		...
		_square_views = MultiArray.new(_level.get_size())
		_add_all_squares()
		_init_squares(_level.squares)
		set_target_pos(_level.target_pos)
		_add_player_robot(_level.start_pos)
		_add_npcs(_level.npcs_pos)
		_resize_squares()


func reset_to_init_pos():
	for robot:Robot in _all_robots:
		robot.reset_to_init_pos()
	select_robot(_get_player_robot().pos)


func get_selected_robot() -> Robot:
	return _all_robots.filter(func(robot): return robot.is_selected)[0]
...
func is_wall_open(current_robot, pos, direction) -> bool:
	var square = _square_views.at(pos.x, pos.y)
...
func _get_player_robot() -> Robot:
	return _all_robots.filter(func(robot): return robot.is_player)[0]



func _ready():
	_square_size = SquareSize.new(_calc_square_size())
	_square_size.size_changed.connect(_on_square_size_changed)
...
func _calc_square_size():
	var board_size = get_size()
	
	return (board_size.x /grid_dimension.x)

func _resize_squares():
	_square_size.set_length(_calc_square_size())
	queue_redraw()


func _add_player_robot(pos:Vector2):
	var player_robot = RobotTemplate.instantiate()
	player_robot.is_player = true
	player_robot.set_square_size(_square_size)	
	player_robot.is_selected = true
	player_robot.set_init_pos(pos)
	_all_robots.append(player_robot)
	add_child(player_robot)
		

func _init_squares(new_squares:Array):	
	for y in new_squares.size():	
		var row = new_squares[y]
		for x in row.size():
			var square_view = _square_views.at(x,y)
			if square_view:
				square_view.init(row[x])
	
	
func _add_npcs(npcs_pos:Array):
	for pos in npcs_pos:
		var npc:Robot = RobotTemplate.instantiate()
		npc.set_init_pos(pos)
		npc.square_size = _square_size
		add_child(npc)
		_all_robots.append(npc)
...
func _remove_all_squares():
	if !_square_views:
		return
	for x in range(_square_views.dimensions.x):
		for y in range(_square_views.dimensions.y):
			_square_views.at(x,y).queue_free()
			_square_views.set_at(x,y,null)
...
func set_target_pos(target_pos:Vector2):
	set_target(_square_views.at(int(target_pos.x), int(target_pos.y)))
...
func _add_npc_robot(pos:Vector2):
	...
	npc.set_square_size(_square_size)
	npc.is_player = false
	npc.set_init_pos(pos)
	_all_robots.append(npc)
	add_child(npc)
